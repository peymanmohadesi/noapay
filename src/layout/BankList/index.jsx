...
import { getUserInfo, getBanks, changeBankState } from '../../api';
...
function BankList() {
    ...
    const addNewCustomer = () => {
        navigate("/add-new-bank")
    }

    const editUser = (userId) => {
        navigate("/edit-bank?bankid=" + userId)
    }
        ...
        <div>
            ...
            <div className={styles.left_side}>
                <div className={styles.search_add}>
                    <div className={styles.add_btn} onClick={addNewCustomer}><FontAwesomeIcon icon={faUserPlus} /> افزودن بانک</div>
                </div>

                <div className={styles.search_container}>
                    <Input 
                        placeholder="جستجو بانک"
                        type="text"
                        value={searchQuery}
                ...
                <div className={styles.customer_list}>
                    <div className={styles.list_header}>لیست بانک‌ها
                        ...
                        <tbody>
                            {customers.map((customer, item) => (
                                <tr>
                                    <td>{item + 1}</td>
                                    <td>{customer.name}</td>
                                    <td>{customer.tranasactionLimit.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td>
